<template>
  <div class="payment-status" v-cloak>
    ...
    <div class="status" v-else>
		<div  v-if="orderInfo.pay_type != 'payh5code'" >订单状态</div>
		<div  v-else>下单成功</div>
    ...
    <div class="wrapper">
      ...
      <div class="item acea-row row-between-wrapper">
		...
		<div class="itemCom" v-if="orderInfo.pay_type == 'payh5code'">
		  扫码付款
		</div>
      </div>
      <div class="item acea-row row-between-wrapper">
	  ...
	  <div class="item acea-row row-between-wrapper" v-if="orderInfo.pay_type == 'payh5code'">
	    <div>收款二维码</div>
...
export default {
  ...
  methods: {
	...
	getPaycode(){
		//TODO test
		var order_id = this.orderInfo.id;
		getProductPaycodebyOrder(order_id)
		.then(res => {
